...
function ProjCard({ project: p, index = 0, pos = 0, grid = false }: any) {
  const src = imgUrl(p)
  ...
  return (
    <div className={`proj-card glass-card${grid ? ' fade-in' : ` sl-card ${pc}`}`}
      ...
      <div className="proj-body">
        ...
        {tools.length > 0 && <div className="proj-tools">{tools.map((t: string, i: number) => <span key={i} className="pt">{t}</span>)}</div>}
