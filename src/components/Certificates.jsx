...
const BUCKET='Portfolio', FOLDER='certificates_image'
const SLIDE_AT=3
...
export default function Certificates(){
  ...
  useEffect(()=>{
    supabase.from('certificate').select('*').order('id',{ascending:false})
      .then(({data,error})=>setCerts(error||!data?[]:data))
      .catch(()=>setCerts([]))
    ...
    <div className="certs">
      <div className="container">
        ...
        {loading?(
          ...
          <div className="cert-slide-wrap" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
            <div className="cert-stage">
              {vis3().map(({c,pos})=>(
                <CertCard key={`${c.id}-${pos}`} cert={c} pos={pos}
            ...
            <div className="cert-dots">
              {certs.map((_,i)=>(
                <button key={i} className={`csd${i===cur?' on':''}`} onClick={()=>{setCur(i);setPaused(true)}}/>
              ))}
            </div>
          ...
          <div className="cert-grid">
            {certs.map((c,i)=><CertCard key={c.id} cert={c} index={i} pos={0} onPreview={()=>setPreview(c)}/>)}
          </div>
        )}
      </div>
      ...
      {preview&&(
        <div className="cert-lightbox" onClick={()=>setPreview(null)}>
          <div className="cert-lb-inner" onClick={e=>e.stopPropagation()}>
            <button className="cert-lb-close" onClick={()=>setPreview(null)}><FiX/></button>
            {imgUrl(preview)
